...
Old users created from Management UI can still log in using the /old-login endpoint. However, it is recommended to migrate these users to utilize the Cognigy.AI OAuth login. To initiate the migration process, follow these steps:
...
>Note: Once this process is completed, the user will no longer be able to log in using `/old-login`, and any changes to their credentials must be made through the Cognigy.AI platform.
