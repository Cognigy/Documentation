...
Agent Assist configurations refer to the settings and layout of the Agent Assist workspace. The workspace can be customized to display various types of content, such as HTML, adaptive cards, or iframes. Agent Assist configurations define the name, description, and layout of the dashboard, as well as the types of content to be displayed. 
...
The Agent Assist workspace is based on a schema in grid form, where each tile can contain HTML, an adaptive card, or an iframe website. You can configure the number of columns and rows, as well as the gaps between them.

## Tile ID

Each tile must have a name, for example, `map`, `profile`, or `history`, that works as a unique identifier and reflects the content of the tile. This tile ID is needed for the Agent Assist Node configuration.
