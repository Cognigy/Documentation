...
- The [ElevenLabs streaming](../voicegateway/webapp/streaming.md) capability is not available. Disable this feature in the Voice Gateway Self-Service Portal: Speech > Elevenlabs > deselect `Use Streaming`
...
- Updated the template for `feature-server` `StatefulSet` and `Deployment`, specifically for the container `freeswitch`:
    ...
    - Use the R70 image of `freeswitch-mrf` in `.Values.freeswitch.image`
