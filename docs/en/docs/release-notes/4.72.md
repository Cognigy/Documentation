...
- Added the Copy to clipboard button to the Copilot: Next Action and Copilot: Knowledge Assist Nodes for all handover providers except Live Agent. For Salesforce, ensure to update your Salesforce integrations using [this guide](https://github.com/Cognigy/salesforce-integrations?tab=readme-ov-file#update)
- Extended the functionality of the [AI Copilot: Next Action](../ai/nodes/ai-copilot/next-action-tile.md) Node to include Knowledge Assist options
...
- Implemented the LLM Entity Extract Node and added the LLM Entity Extraction Options to the Question Node
...
- Added the ability to select a Handover provider for the Voice Gateway Endpoint. After Handover initiation, a gather verb is sent to collect transcripts and send them to the human agent
...
- Added the UI element to change the size of the conversation chat interface
