...
- Improved the Salesforce Service Cloud Handover Provider settings by adding the Button ID field to the Handover to Agent Node, so now you can use different Button IDs for individual Handover to Agent Nodes
...
- Added the capability to use variables in [canned responses](../live-agent/settings/canned-responses.md) and [messages](../live-agent/conversation/conversation-workflow.md#send-a-response-with-variables)
