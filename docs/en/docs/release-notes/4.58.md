...
- Improved by adding the support for receiving RCE queue updates through the lookup node with the Queue handover status
...
- Fixed the issue where Duplicate Sessions in OData were observed
- Fixed the issue where the LiveAgentEscalations OData feed returns an error in Excel
- Fixed the issue where the Session Detail panel crash in case of missing profile information
...
- Added the capability to record calls, store them in an AWS S3 bucket, and review call recordings directly in the Recent Calls section of the Voice Gateway Self Service Portal
