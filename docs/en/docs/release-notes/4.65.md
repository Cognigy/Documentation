...
- Fixed the issue that caused slowness while reading resources in the agent when Knowledge AI Chunks were ingested in the background

#### Improvements

- Added the retry mechanism for retrieving the embedding vectors from the Generative AI provider
- Increased default resources for the synchronisation KAI data job.
- Improved by making it possible to set the replication factor for the Weaviate schema via an environment variable- Improved by making it possible to set the replication factor for the weaviate schema via an environment variable.
...
- Added the [Smart Formatting and Endpointing](../ai/flow-nodes/vg/transfer.md#transcribe) options for the Deepgram STT provider
