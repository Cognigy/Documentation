...
- Embedded the [Agent Assist Workspace](../ai/handover-providers/la-agent-assist-workspace-setup.md) functionality into Live Agent. This feature is in Beta

### Infrastructure

#### New services
This release contains Cognigy xApps in a public beta - the xApps microservices will be deployed by default. Upgrading to this release will:
- add two new Kubernetes Deployments `serviceAppSessionManager` and `serviceStaticFiles`
- two new Kubernetes Services for the Deployments (see above)
- two new Ingress objects for the Pods created by the Deployments (see above)

You have two create two new DNS entries for the Ingress objects above and point them to your loadbalancer which was provisioned when you have installed Cognigy.AI. Here is an example of how you could name those DNS names if your `Cognigy.AI UI` is located on `trial.cognigy.ai`:
- for service-static-files: `static-trial.cognigy.ai`
- for service-app-session-manager: `apps-trial.cognigy.ai`

Ensure that the records have been properly created by using a command line tool like `dig` on Linux.

The last step is to remove the `cognigyApps.enabled` feature flag from your own `values` file in case you have ever added it to it:
```yml
...
This is no longer required and the condition won't be used even if present.