...
- Added using the last [conversation entries](../ai/nodes/service/llm-prompt.md#additional-tags) in the GPT Prompt Node. To do that, specify `@cognigyRecentConversation` or `@cognigyRecentUserInputs` in the Prompt field
- Renamed xApp user-facing labels from `app/apps` to `xApp/xApps`, and the names of the [xApp Nodes](../ai/nodes/xApp/overview.md) was also changed
