...
???+ warning "Attention!"
    The Alexa Endpoint uses the Alexa NLU Connector. See the [NLU Connectors Page]({{config.site_url}}/ai/resources/build/NLUconnectors/) for more details. When using Amazon Alexa with Cognigy, all NLU is done by Amazon. **It is NOT possible to use the Cognigy NLU**.
