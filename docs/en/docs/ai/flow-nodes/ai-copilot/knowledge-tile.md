...
The Node creates an AI Cpilot widget that consists of a search bar and a response form.
...
The human agent uses the search bar to access your knowledge base and find information to respond to user queries.
...
To set the Node up, you need two Nodes: one to initiate the knowledge search process and the second one to execute it. Additionally, to prevent the content of other widgets from updating during the Node's operation, move the Nodes under the child nodes of the IF Node. For more information, refer to the [Configuration](#configuration) section.
...
3. In the **Condition** field, specify the following condition: `input.data._cognigy._agentAssist.payload.tileId === "tile-id"`, where `tile-id` is a Knowledge tile ID that you created in the AI Copilot configuration. 
