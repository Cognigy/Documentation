...
**Endpoints** help connect **Channels **(like Messenger or Alexa), to specific Cognigy [**Flows**]({{config.site_url}}ai/resources/build/flows/). They make sure that the **channel-specific input and output** (Images, Galleries, Voice) gets converted to the standardized [**Input**]({{config.site_url}}ai/tools/interaction-panel/input/)  object. This way, a Cognigy Flow needs to be developed only once, after which it can be deployed to many different channels by configuring Endpoints.
...
|**Azure Bot Services**    | The [Azure Bot Services](doc:azure-bot-services) Endpoint allows you to connect your Virtual Agent to all Azure Bot Service channels. |
