...
2. Allow users to access the xApp Session using one of the following methods:
    - **Without entering a PIN Code.** Share the xApp Session URL with the users. The URL can be sent in the Flow with a [xApp Session URL](../flow-nodes/xApp/tokens.md#xapp-session-url) Cognigy token.
    - **By entering a PIN Code.** Require a PIN and URL with users to access the xApp Session.
      ...
      2. Share the xApp PIN Page URL with the users, where they can enter the PIN. The URL can be sent in the Flow with a [xApp PIN Page URL](../flow-nodes/xApp/tokens.md#xapp-pin-page-url) Cognigy token.
