...
With release 4.73.0, Cognigy.AI transitions to using Qdrant as the vector database instead of Weaviate.
...
2. Review the report generated by the migration job to determine which projects, if any, will be affected.

Note that following such a migration,
the Qdrant Persistent Volume Claims (PVCs) and Persistent Volumes (PVs) will need to be deleted. For details, see the [Emptying Qdrant](#emptying-qdrant).
...
- This guide assumes that Cognigy.AI is deployed in the `cognigy-ai` namespace. If you've deployed Cognigy.AI in any other namespace, then adjust the namespace parameters in all the commands accordingly.

## Prerequisites

- Cognigy.AI v4.73 is deployed.
- Kubernetes v1.21-1.27.
- `kubectl` utility installed locally on Linux or macOS client host. This guide doesn't support Windows client hosts.
...
The following equation can be used to calculate the memory required by each Qdrant pod in megabytes:
...
Before starting the process of migrating from Weaviate to Qdrant, 
it is recommended that you create a test project,
...
Before starting to populate Qdrant, make sure all three Qdrant pods are up and running and Weaviate has been scaled down. If you are running a test migration, weaviate should not be scaled down.

To populate Qdrant with the existing Knowledge AI data, a `Kubernetes Job` must be run in the cluster. The time it takes for this job to finish depends on the amount of Knowledge AI data that needs to be copied, as well as the total number of projects/agents that are configured to use Knowledge AI.
...
```json
{
  "Organisation: <org-id>": {
    "Project: <proj-id>": {
      "ignoredSource": 2, // Total No. of ignore sources
      "Source: <source-id>": <No. of chunks ignored in this source>,
      "Source: <source-id> ": <No. of chunks ignored in this source>
    },
    "Project: <proj-id>": {
      "missingEmbedding": <No. of chunks ignored>
    }
  }
}
```
...
1. You ran a test migration which populated qdrant. Since the migration job only works with an empty Qdrant database, you need to remove all data from Qdrant before starting a proper migration.
2. As part of a proper migration, you finished running the migration job but have not yet started Step 3. Due to a potential issue with the migration job, you would like to run it again before starting Step 3.
