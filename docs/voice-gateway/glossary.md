...
: A parameter within the Session Config Node, which allows the user to configure a background track to be played during the conversation. An Atmosphere Sounds track can be played, silenced or stopped at any time during the Voice Gateway conversation. You can lower the volume of the track so that it doesn't overwhelm the conversation if too loud, or make it louder if it's not audible. For more information, read the [Voice Gateway Parameter Details](../ai/build/node-reference/voice/voice-gateway/parameter-details.md) documentation.
