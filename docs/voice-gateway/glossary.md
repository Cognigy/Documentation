...
_**Silence Overlay**_
: A Voice Gateway feature which allows the user to play an MP3 track wherever an extended silence occurs, for example while the AI Agent is gathering data or looking for specific results in the database. It can be used to simulate common call center ambient noise, such as typing the customer's information into the system, or mouse-clicking, suggesting to the customer that the agent is working. For more information, read the [Silence Overlay]() documentation.
