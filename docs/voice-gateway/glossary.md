...
_**Silence Overlay**_
: A parameter within the Session Config Node, which allows the user to play an MP3 track wherever an extended silence occurs due to AI Agent's background activity, for example while the AI Agent is gathering data or looking for specific results in the database. It can be used to simulate common call center ambient noise, such as typing the customer's information into the system, or mouse-clicking, which suggests to the customer that the agent is working. For more information, read the [Silence Overlay](../ai/build/node-reference/voice/voice-gateway/parameter-details.md#silence-overlay) documentation.
