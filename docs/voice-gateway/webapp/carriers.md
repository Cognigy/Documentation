...
Before 4.80 release, all connections were encrypted with SIPS by default. In some cases, the SIPS default setting could cause compatibility issues. Some connection nodes, such as routers, internet switches, or service providers, still use older systems that don't support SIPS, causing the connection to fail. Starting from release [4.80](../../release-notes/4.80.md), the users can choose whether they want SIP, SIP over TLS, or SIPS encryption. All new carriers have SIP connection scheme by default. The existing carriers will retain their SIPS scheme.
