...
| Display name  | This name will be displayed in the conversations and is used as sender name when human agents send conversation transcripts in emails.<br> For [Webchat v3](../webchat/v3/configuration.md#webchat-layout), when the  Display name is not available the system will use the value from the Human Agent Avatar Logo URL parameter in the Webchat Layout section if it is provided. Otherwise, the value of the **Name** parameter in the Webchat Layout section will be displayed. |
