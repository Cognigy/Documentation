...
This release extends the list of supported Large Language Model providers and models that can be used for our Agentic AI Agents.

We have extended our already existing [Google Gemini](../ai/empower/llms/providers/google.md) integration and added [support](../ai/empower/llms/model-support-by-feature.md) for the `gemini-1.5-pro` and the `gemini-1.5-flash` models for recent models Google has integrated. 

Additionally we have extended our [AWS Bedrock](../ai/empower/llms/providers/amazon-bedrock.md) integration to fully support the new `Nova` model series from AWS.
...
This release introduces the ability to [pin Projects](../ai/build/projects.md#pin-and-unpin-projects) and better organise what you see when you sign-in into Cognigy.AI. The list of pinned Projects will be stored in the users profile and we have plans to extend the list of Resources users can pin in the future.

##### Download for Knowledge Sources in CTXT
This release adds the capability to download content stored in Knowledge Sources in the Cognigy `CTXT` format. This allows our customers to better share the content stored in our Knowledge AI product across individual teams and projects. Our exporter will keep the actual structure of your Knowledge Chunks so re-imports of the generated files will generate the same Chunk structure.

#### Improvements
- Increased the button limit from 6 to 15 in the [Text with Buttons](../ai/build/node-reference/basic/say.md#text-with-buttons) output type
...
- Added the capability to [track total token usage](../ai/empower/llms/llm-session-token-counter.md) in a conversation or chat session via a Node API. This capability allows users to monitor their LLM token usage per session
...
- Added the Generate Search Prompt parameter to the Grounding Knowledge section in the [AI Agent Node](../ai/build/node-reference/ai/ai-agent.md#grounding-knowledge). This parameter is enabled by default and allows you to generate a context-aware search prompt before executing the knowledge search
...
- Added the capability to set `recognizer`, `fillerNoise`, and `synthesizer` via the [Advanced Session Parameters](../ai/build/node-reference/voice/voice-gateway/parameter-details.md#list-of-additional-session-parameters) in the Set Session Config Node and via the Set Activity Parameters in the Say or Question Nodes
...
- Added support for images to the [Text with Quick Replies](../ai/build/node-reference/basic/say.md#text-with-quick-replies) output type. This improvement applies to Webchat v3
