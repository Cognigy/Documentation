...
- Fixed the issue in the the vg-drachtio-server where a part in the entrypoint was missing - leading to unresolved IP addresses in Azure clusters using a load balancer 
- Improved handling of TTS audio files in the cache by creating a new namespace based on their file extension

### Infrastructure

#### Cognigy Voice Gateway

- Added the environment variable PRIVATE_VOIP_NETWORK_CIDR to sbc-inbound and sbc-outbound Deployments. It can be set via {{ .Values.global.privateVoipNetworkCidr }}. This variable can be used for a private network (e.g. '172.31.0.0/16')
- Increased api-server rate limit to 1000 within 4 minutes
