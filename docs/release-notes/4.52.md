...
- Added the [Genesys section](../ai/escalate/handover-reference/genesys-cloud-guest-chat.md) to the Handover to Agent Node to extend it with 3 new fields and a JSON field to send Custom attributes when doing the handover
...
- Fixed the issue where the [maximum Slot limit](../ai/empower/nlu/slots/user-defined/lexicon.md#limitations) allowed for the lexicon was not considered during lexicon CSV upload. To support backward compatibility, we made this limit configurable
