...
- Removed the Chatbase analytics configuration from the Endpoint Settings, as the Chatbase no longer exists
- Improved by not showing the Enable Input Sanitization field in Endpoint Settings if the feature is not globally activated
...
- Fixed the issue where inputs were processed incorrectly or redundantly when using Input Transformers
