...
Introduced AI Agent child Nodes: [AI Agent MCP Tool](../ai/build/node-reference/ai/ai-agent.md#ai-agent-mcp-tool-settings) and [Call MCP Tool](../ai/build/node-reference/ai/ai-agent.md#call-mcp-tool-settings). Along with tools, you can use Model Context Protocol (MCP), which boosts tools by providing a standardized connection between AI Agents and external services. It allows smooth integration with remote systems like APIs and databases, enabling the AI Agent to perform more complex tasks in a secure and efficient way.
