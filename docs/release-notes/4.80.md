...
- Fixed a bug where a handover to Live Agent was failing and could not transmit any messages
...
- Simplified the `update_last_seen` endpoint to avoid extra calculations

### Improvements

- Enhanced caching strategy by configuring Redis to improve performance
- Reduced the frequency of requests to the Conversations Meta Endpoint by applying request throttling
- Improved caching efficiency with short-term caching and consistent ETag generation
- Implemented a lighter liveness endpoint to provide accurate pod health status
- Optimized the performance of availability checks by running them in worker processes threads and introducing new locking logic 
- Enhanced the performance of assignable agents endpoint

### Infrastructure

- Introduced database read replicas to improve infrastructure reliability
...
- Added the capability to configure the [Azure AI Document Intelligence](../ai/empower/knowledge-ai/text-extraction/text-extraction-with-azure-ai.md) connection for text extraction within the Cognigy.AI UI
