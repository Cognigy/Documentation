...
- Enable the environment variable `VG_TTS_TRIM_SILENCE`. It can be controlled via `.Values.featureServer.trimTts`. The default value is `true`. This trims trailing silence (zero-amplitude samples) from the end of an audio file (Microsoft TTS only)
