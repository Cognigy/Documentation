...
1. Create an account in one of the LLM Providers:
    - [OpenAI](https://platform.openai.com/). You need to have a paid account or be a member of an organization that provides you access. Open your OpenAI user profile, copy the existing API Key, or create a new one and copy it. You can also use an [OpenAI-compatible LLM provider](../llms/providers/openai-compatible.md) that supports the OpenAI API.
...
2. Add two models:
    - A primary model to control the AI Agent, selected from the following providers:
        - `gpt-4o` or `gpt-4o-mini` powered by [OpenAI](../llms/providers/openai.md) or [Azure OpenAI](../llms/providers/microsoft-azure-openai.md). You can also use an OpenAI-compatible model.
        - `claude-opus-4-0`, `claude-sonnet-4-0`, or `claude-3-haiku` by [Anthropic](../llms/providers/anthropic.md).
