...
The [LLM Entity Extract Node](../build/node-reference/other-nodes/llm-entity-extract.md) retrieves specific entities from user inputs, such as product or booking codes and customer IDs. For example, if a user says, `I would like to book flight AB123 from New York to London`, the LLM can extract the booking code `AB123` from the input.
