...
This Node creates a widget in the Agent Copilot workspace to securely request an end user's credit card information.
...
The Node must be integrated into the Agent Copilot Flow, requiring only the Tile ID for configuration.
...
1. A human agent initiates the process by clicking a button in the Agent Copilot widget.
