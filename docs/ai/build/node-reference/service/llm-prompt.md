...
To store the model output, select **Store to Input** or **Store to Context** in the [Storage & Streaming Options](#storage-and-streaming-options) section.

Additionally,
you can display stored output to the user
by enabling the [Output result immediately](#storage-and-streaming-options) option or using the Say Node and LLM Prompt token.

### Streaming Results

In Stream mode, LLM generates tokens and returns them one by one to Cognigy to ensure low-latency responses.
Cognigy monitors for delimiter tokens ([Stream Buffer Flush Tokens](#storage-and-streaming-options)),
...
[Stream Buffer Flush Overrides](#storage-and-streaming-options) determine if we should <b>not</b> flush the stream buffer by running certain regex. There are pre-configured overrides, but further can be set in the [options](#storage-and-streaming-options).

Pre-configured Stream Buffer Flush Overrides:
...
| Stream Buffer Flush Tokens        | Text Array | The parameter appears when **Stream to Output** is selected. Tokens after which to flush the stream buffer to the output. The tokens can be punctuation marks or symbols, such as `\n`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Stream Buffer Flush Overrides        | Text Array | The parameter appears when **Stream to Output** is selected. Regular Expressions without leading and trailing forward slashes to override stream buffer flushing. Trailing `$` are added automatically to check only against the end of the buffer. E.g. `\d+\.` checks whether the end of the string contains a number followed by a dot.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
1. [Create an Anthropic LLM resource](../../../empower/llms.md#add-a-model) for Claude-1, for example, `claude-3-opus-20240229`.
