...
Before using this Node, configure a model in the [LLM](../../../empower/llms.md) settings.
...
| Stream Reprompt to Output | Toggle        | Streams the result directly into the output. This means that the model provides prompts directly into the conversation chat, and you don't need to use the LLM Prompt Result token and Say node. Note that streaming may not be supported by all [Cognigy LLM Prompt Node](../../../empower/llms.md#supported-models) providers, such as Google[^*]. |
