...
- To activate this feature, use the `FEATURE_USE_MILESTONES` feature flag.
...
6. In the **Steps** section, click **+ Create New Step** and add at least one step to complete the goal configuration.
7. Configure the Step parameters:
    ...
    - **Mark Step as Total Completion Point** — activate this setting to mark the current step as the point where the goal is considered fully completed. For example, this step can reflect the key result when the AI Agent has resolved all user requests. Note that at least one step within the goal must be marked as completed. Multiple steps can be marked as completed.
...
Place the Goal Nodes after each user action where users submit information,
