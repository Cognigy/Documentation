...
- The default number of steps per goal is 10. You can increase this number to 20 by using the `FEATURE_USE_MILESTONES__MAX_STEPS_PER_MILESTONE` variable in the `values.yaml` file.
...
4. On the **Goal** page, configure the parameters of a goal:
    - **Goal Name** — enter a name that reflects the goal. For example, `Reduce Call Handling Time` or `Increase Automation Efficiency`.
...
5. In the **Startpoint-Step** section, complete the following fields:
    - **Name** — provide a descriptive name for the starting step. For example, `Initiate AI Agent Conversation`.
...
6. In the **Steps** section, click **+ Create New Step** and add at least one step to complete the goal configuration. Note that if you add only one step, it must be marked as a Completion Point.
7. Configure the Step parameters:
    ...
    - **Completion Point** — activate this setting to mark the current step as the point where the goal is considered fully completed. For example, this step can reflect the key result when the AI Agent has resolved all user requests. Note that at least one step within the goal must be marked as completed. Multiple steps can be marked as completed.
8. In the **Success Metrics** field,  measure the impact of your goal by tracking the following performance metrics:
    - **Money Saved** — enter values in the format `xx.xx` (for example, `150.00` for `$150`). Make sure your currency are configured in the **Currency Settings** section within the [Project Settings](#determine-what-you-want-to-measure).
    - **Time Saved** — select the time measurement unit (minutes or hours) and specify the time saved. Enter values in the format `xx.xx`:
        - For minutes, the digits before the decimal point represent minutes, and the digits after represent seconds. For example, `12.30` (12 minutes and 30 seconds).
        - For hours, the digits before the decimal point represent hours, and the digits after represent minutes. For example. `2.45` (2 hours and 45 minutes).
9. Click **Save** to complete your goal configuration.
...
6. Configure the Node settings:
    ...
    - **Select Steps** — choose the steps within the goal that you want to track. Note that the first selected step must be the Startpoint step. Set the Startpoint step at the beginning of the process you want to track in your Flow. Otherwise, the goal will not be tracked.
