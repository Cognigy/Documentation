...
8. _(Optional)_ Activate the **Accept Conversation Active Event** setting. When activated, the setting allows the system to accept and process `CONVERSATION UPDATE` events with the state set as `ACTIVE`. These events will be sent to the Flow for further handling. This setting allows accessing the event payload via the [Input object](../../build/ai-agent-memory/input.md), which provides relevant information related to the ongoing conversation.
